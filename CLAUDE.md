...
- `js/level.js` の `generateLevel(lv)` は、スタートから部屋を1つずつ広げて迷路（木構造）を作り、その順を「正解の順番」として数値を決める。近道は `CONFIG.loopRate`。💣・☠ はボスへの幹の道に優先して置く。爆風で順番が変わることもあるので、最後に `bestScore` で解けるか確かめ、解けなければ作り直す → 必ずクリア可能。この性質は壊さないこと。
