...
- 移動ルール：通ったエリア（スタート地点 'home' ＋クリア済みマス）に隣接するマスにだけ進める。隣接 = 同じ塔の上下の階、隣の塔の同じ階（`isReachable` / `neighborsOf`）。home は (0,0) とつながる。
- `js/level.js` の `generateLevel(lv)` は「正解の順番」を先に作り、スタートから隣接マスを1つずつ広げた順に配置する → 正解の順番どおりなら常に隣へ進めるので必ずクリア可能。この性質は壊さないこと。ボスは最後の塔のてっぺんで最後に到達するマス。
